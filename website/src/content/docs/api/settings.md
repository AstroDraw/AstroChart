...
| `COLORS_SIGNS`      | `string[]` | _(array of the 12 above in order)_ | Ordered array of all 12 sign sector colors |
...
| Setting   | Type                                                               | Default       | Description                                                                                                |
| --------- | ------------------------------------------------------------------ | ------------- | ---------------------------------------------------------------------------------------------------------- |
| `ASPECTS` | `Record<string, { degree: number, orbit: number, color: string }>` | _(see below)_ | Aspect definitions. Each key is an aspect name; the value defines its degree, allowed orb, and line color. |
...
| `d`              | Detriment        | The planet is in the opposite sign of its rulership — weakened position.  |
...
| `f`              | Fall             | The planet is in the opposite sign of its exaltation — weakened position. |

All dignity labels are configurable. Set `SHOW_DIGNITIES_TEXT` to `false` to hide them entirely.
...
| `DIGNITIES_EXACT_EXALTATION_DEFAULT` | `Dignity[]` | _(Crowley positions)_ | Array of `{ name, position, orbit }` defining exact exaltation degrees |
