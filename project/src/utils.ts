...
import { Settings } from "./settings";
	 ...
	 */
	export const getPointPosition = ( cx: number, cy: number, radius: number, angle: number, astrology: { SHIFT_IN_DEGREES: number; } ) => {
	...
	export const degreeToRadians = ( degrees: number ) => degrees * Math.PI / 180;

	export const radiansToDegree = ( radians: number ) => radians * 180 / Math.PI;
...
interface TextLocation { text: string; x: number; y: number; }
	 ...
	 */
	 export const validate = ( data: AstroData ) => {
	 ...
	 * @return {DOMElement}
	 */
	 export const getEmptyWrapper = ( parent: Element, elementID: string, _paperElementId: string ) => {
	...
	*/
	 ...
	 *
 	 ...
 	 * @return {boolean}
	 */
	 export const isCollision = (circle1: { x: number; y: number; r: number; }, circle2: { x: number; y: number; r: number; }) => {
	 ...
	 */
	 export const assemble = ( locatedPoints: LocatedPoint[], point: LocatedPoint, universe: { cx: number; cy: number; r: number; }, astrology: Settings) => {
	 ...
	 */
	 export const placePointsInCollision = (p1: LocatedPoint, p2: LocatedPoint) => {
	 ...
	 *
 	 ...
 	 * @return {boolean}
	 */
	 export const isInCollision = (angle: number, points: string | any[], astrology: Settings) => {
	 ...
	 */
	 export const getDashedLinesPositions = ( centerX: number, centerY: number, angle: number, lineStartRadius: number, lineEndRadius: number, obstacleRadius: number, obstacles: LocatedPoint[], astrology: Settings) => {
		let startPos
		let endPos
		const result = [];
	 ...
	 */
	 export const getRulerPositions = ( centerX: number, centerY: number, startRadius: number, endRadius: number, startAngle: number, astrology: { SHIFT_IN_DEGREES: number; } ) => {
		const result = [];
	...
	* @return
	*/
	export const comparePoints = ( pointA: { angle: number; }, pointB: { angle: number; }) => {
