...
(function( astrology ) {
	...
	function sun( x, y ){
		...
		var xShift = -10; //px						
		var yShift = 0; //px		
		x =  Math.round(x + (xShift * astrology.RADIX_SYMBOL_SCALE));
		y =  Math.round(y + (yShift * astrology.RADIX_SYMBOL_SCALE));
		...
		node.setAttribute("transform", "translate(" + ( -x * (astrology.RADIX_SYMBOL_SCALE - 1)) + "," + (-y * (astrology.RADIX_SYMBOL_SCALE - 1)) + ") scale(" + astrology.RADIX_SYMBOL_SCALE + ")");									
		return node;
	};
