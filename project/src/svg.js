...
(function (astrology) {
	...
	function sun(x, y) {
		...
		const name = "sun";
		var accessor = astrology.utils.getIconReference(name);
		...
		if (astrology[accessor] !== "") {
			// reset pos
			x = Math.round(x + (-xShift * astrology.SYMBOL_SCALE * 0.5) * astrology.SYMBOL_SCALE);
			y = Math.round(y + (-yShift * astrology.SYMBOL_SCALE * 0.5) * astrology.SYMBOL_SCALE);
			astrology.utils.getCustomIcon(astrology[accessor], x, y, wrapper, name);
		} else {
